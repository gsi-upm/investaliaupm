...
public class MessagesFromAPI {
	 ...
	 * @param username
	 * @param password
	 * @return true if the authentication is succesful.
	 */
	...
	public static List<Message> getBlogsFromAPI (String lastUpdate){
		...
		try{
			...
			for(int i = 0;i<blogs.length();i++){
				...
				tags.add(new Tag(0, "prueba")); // TODO: abajo está puesto el código para cogerlas, pero con los ejemplos no funciona
				
				/*String tags = blogs.getJSONObject(i).getString("tags");
				JSONArray tags_j = blogs.getJSONObject(i).getJSONArray("tags");
				List<Tag> tags = new ArrayList();
				for(int j=0;j<tags_j.length();j++){
					tags.add(new Tag(0, tags_j.getJSONObject(j).toString()));
					System.out.println(tags.toString());
				}*/
				
				
				Date date = new Date();
				//TODO: debería ser Date date = blogs.getJSONObject(i).getString("time_created); y algún tipo de parseado.
				boolean read =false;
				boolean liked = false;
				int rating= Integer.parseInt(blogs.getJSONObject(i).getString("rate")); //TODO: con uno de los blogs da error, no sé por qué. Con el resto va bien.
				int timesRead = 0; //Por defecto, 0
				int idUserUpdating = 0; //TODO: qué es esto?
